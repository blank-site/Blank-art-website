...
class MostPopularTags extends WP_Widget
{
    function __construct()
    {
        parent::__construct('tagmostpopular','ET Most Popular Tags');
    }
    function form($instance)
    {
        $defaults = array(
            'top_count' => 10,
            'caption' => 'Most popular tags',
            'hours_ago' => 1,
            'minutes_ago' => 0
        );
        ...
        <p>
            <label for="<?php echo $field1_id; ?>">
                <?php _e('How many popular tags to display:','easytagger'); ?> <br/>
    ...
    {    
        ...
        <?php
        $entries = $etdb->get_most_popular_tags(
           array(
               'tag_count' => $top,
               'hours_ago' =>$hr,
               'minutes_ago' => $min
        ...
        if(!$entries){
           ...
           foreach($entries as $e){
                ...
                <p>
                    ...
                    <?php _e('tagged by','easytagger'); ?>
                    <strong><?php echo $e['user_count']; ?></strong>
                    <?php if($e['user_count'] > 1)
                            _e('users','easytagger');
                          else
                            _e('user','easytagger');   
                   ?>
...
add_action(
        'widgets_init',
        create_function('','register_widget("MostPopularTags");')
);
